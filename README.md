...
```cpp
std::vector<int> input = {1, 2, 3, 4, 5};
std::vector<int> result = trx::from(input)
...
std::vector<std::string> result = trx::from(input)
...
std::string result = trx::from(input)
    |= trx::intersperse(',')
    |= trx::into(std::string{});
// result: "A,B,C,D"
```
...
```cpp
std::vector<int> input = {1, 2, 3, 4, 5};
auto [first, second] = trx::from(input)
    |= trx::partition(
        [](int x) { return x % 2 == 0; },
        trx::into(std::vector<int>{}),
        trx::into(std::vector<int>{}));
...
```cpp
std::vector<int> input = {1, 2, 3, 4, 5};
std::vector<int> result = trx::from(input) |= trx::into(std::vector<int>{});
// result: {1, 2, 3, 4, 5}
```
...
```cpp
std::vector<int> input = {1, 2, 3, 4, 5};
std::size_t result = trx::from(input) |= trx::count;
// result: 5
```
